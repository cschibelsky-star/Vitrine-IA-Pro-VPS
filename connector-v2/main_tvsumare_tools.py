...
def _request(method: str, path: str, payload: dict[str, Any] | None = None, timeout: float | None = None) -> dict[str, Any]:
    ...
    Regra de arquitetura: somente o FastMCP principal do conector registra tools.
    Este módulo contém exclusivamente funções Python puras para evitar registries
    duplicados e comportamento diferente entre descoberta e execução.
    """
    url = f"{OPS_BROKER_URL}{path}"
    try:
        with httpx.Client(timeout=timeout or OPS_REQUEST_TIMEOUT) as client:
            response = client.request(method, url, headers=_headers(), json=payload if payload is not None else None)
