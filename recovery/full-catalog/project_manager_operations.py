...
ALLOWED_WORKSPACE_ROOTS = tuple(Path(item.strip()).resolve() for item in os.getenv("PROJECT_WORKSPACE_ROOTS", "/srv/tvsumare,/srv/projects").split(",") if item.strip())
DOCKER_ALLOWED_PREFIXES = tuple(item.strip() for item in os.getenv("PROJECT_DOCKER_ALLOWED_PREFIXES", "vitrine_core_,cursos_ia_mvp_,tvsumare_,agente_compras_").split(",") if item.strip())
SENSITIVE_ENV_MARKERS = ("PASSWORD", "PASSWD", "SECRET", "TOKEN", "API_KEY", "APIKEY", "PRIVATE_KEY", "ACCESS_KEY", "AUTH", "CREDENTIAL")
...
def audit(action: str, project_id: str, payload: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    record = {"at": datetime.now(timezone.utc).isoformat(), "scope": "project-manager", "action": action, "project_id": project_id, "payload": payload, "result": result}
...
def project_clone(req: ProjectRequest) -> dict[str, Any]:
    ...
    if (target / ".git").is_dir():
        ...
        if not fetch["ok"]:
            result = {"ok": False, "stage": "fetch", "result": fetch}; audit("project_clone", req.project_id, req.model_dump(), result); return result
        checkout = run(["git", "checkout", branch], target)
        if not checkout["ok"]:
            result = {"ok": False, "stage": "checkout", "result": checkout}; audit("project_clone", req.project_id, req.model_dump(), result); return result
        pull = run(["git", "pull", "--ff-only", "origin", branch], target)
        result = {"ok": pull["ok"], "operation": "updated", "project_id": req.project_id, "repository": str(target), "origin": url, "origin_corrected": current_origin != url, "result": pull}
        audit("project_clone", req.project_id, req.model_dump(), result); return result
    if target.exists() and any(target.iterdir()):
        result = {"ok": False, "stage": "preflight", "error": "repository_directory_not_empty", "repository": str(target)}; audit("project_clone", req.project_id, req.model_dump(), result); return result
...
def project_status(project_id: str) -> dict[str, Any]:
    ...
    return {"ok": True, "project_id": project_id, "workspace_exists": root.exists(), "repository_exists": repository.exists(), "repository_is_git": (repository / ".git").is_dir(), "git_status": status, "origin": origin}
...
def project_docker_container_info(req: ProjectContainerRequest) -> dict[str, Any]:
    manifest = load_manifest(req.project_id); root, _, _ = project_paths(manifest); name = validate_container_name(req.container_name); inspected = run_json(["docker", "inspect", name], root)
    ...
    result = {"ok": True, "project_id": req.project_id, "container": {"name": str(item.get("Name", "")).lstrip("/"), "image": item.get("Config", {}).get("Image"), "status": item.get("State", {}).get("Status"), "running": bool(item.get("State", {}).get("Running")), "health": item.get("State", {}).get("Health", {}).get("Status"), "networks": {network_name: {"ip_address": network_data.get("IPAddress"), "gateway": network_data.get("Gateway"), "aliases": network_data.get("Aliases") or []} for network_name, network_data in networks.items()}, "ports": item.get("NetworkSettings", {}).get("Ports", {}) or {}}}
...
def project_docker_container_env_safe(req: ProjectContainerRequest) -> dict[str, Any]:
    ...
    result = {"ok": True, "project_id": req.project_id, "container_name": name, "environment": redact_container_env(env_items)}
