...
def _request(method: str, path: str, payload: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    with httpx.Client(timeout=OPS_REQUEST_TIMEOUT) as client:
        response = client.request(method, f"{OPS_BROKER_URL}{path}", headers=headers, json=payload)
    try:
        body = response.json()
    except ValueError:
...
def project_docker_container_info(project_id: str, container_name: str) -> dict[str, Any]:
    return _request("POST", "/projects/docker/container-info", {"project_id": project_id, "container_name": container_name})


def project_docker_container_env_safe(project_id: str, container_name: str) -> dict[str, Any]:
    return _request("POST", "/projects/docker/container-env-safe", {"project_id": project_id, "container_name": container_name})
