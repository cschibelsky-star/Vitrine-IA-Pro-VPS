...
function ds_once(&$code,$old,$new,$label){ if(strpos($code,$new)!==false) return; $n=substr_count($code,$old); if($n!==1){fwrite(STDERR,"{$label}: trecho esperado count={$n}; abortando\n");exit(2);} $code=str_replace($old,$new,$code); }
ds_once($code,"function ds_clean(\$v){ return trim(preg_replace('/\\s+/u',' ',strip_tags((string)\$v))); }","function ds_clean(\$v){ return trim(preg_replace('/\\s+/u',' ',strip_tags((string)\$v))); }\nfunction ds_status_label(\$s){ return ['aguardando_integracao'=>'Na fila','na_fila'=>'Na fila','processando'=>'Publicando','publicado'=>'Publicado','falhou'=>'Falhou','cancelado'=>'Cancelado'][strtolower((string)\$s)]??ucfirst(str_replace('_',' ',(string)\$s)); }\nfunction ds_queue_active(\$q){ return in_array(strtolower((string)(\$q['status']??'')),['aguardando_integracao','na_fila','processando'],true); }",'Social helpers');
ds_once($code,"\$caption=trim((string)(\$_POST['caption']??'')); \$hashtags=trim((string)(\$_POST['hashtags']??'')); \$engine='manual';","\$caption=trim((string)(\$_POST['caption']??'')); \$hashtags=trim((string)(\$_POST['hashtags']??'')); \$engine=(\$caption!=='' && \$hashtags!=='')?'revisado':'automatico';",'Social reviewed copy');
ds_once($code,"\$duplicate=false; foreach(\$queue as \$q){ if((\$q['video_id']??'')===\$videoId && in_array((\$q['status']??''),['aguardando_integracao','na_fila','processando'],true)){ \$duplicate=true; break; } }","\$duplicate=false; foreach(\$queue as \$q){ if((\$q['video_id']??'')===\$videoId && ds_queue_active(\$q)){ \$duplicate=true; break; } }",'Social idempotency');
if(strpos($code,"array_filter(\$videos,function(\$v) use (\$queue)")===false) ds_once($code,"\$ready=array_values(array_filter(\$videos,'ds_social_ready'));","\$ready=array_values(array_filter(\$videos,function(\$v) use (\$queue){ if(!ds_social_ready(\$v)) return false; foreach(\$queue as \$q){ if((\$q['video_id']??'')===(\$v['id']??'') && ds_queue_active(\$q)) return false; } return true; }));",'Social ready filtering');
ds_once($code,"<div class=\"box\" style=\"margin-top:14px\"><h2>Novo envio</h2><?php if(!\$ready): ?><p>Nenhum vídeo vertical 9:16 do Repórter IA está pronto no momento.</p>","<div class=\"box\" style=\"margin-top:14px\"><h2>Preparar publicação</h2><?php if(!\$ready): ?><p>Nenhum vídeo pronto aguardando preparação. Vídeos já colocados na fila não aparecem novamente aqui.</p>",'Social heading');
ds_once($code,"<?=ds_h(\$q['status']??'')?>","<strong><?=ds_h(ds_status_label(\$q['status']??''))?></strong><?php if(!empty(\$q['network_status'])): ?><br><small><?php foreach(\$q['network_status'] as \$net=>\$st): ?><?=ds_h(ucfirst(\$net))?>: <?=ds_h(ds_status_label(\$st))?> <?php endforeach; ?></small><?php endif; ?>",'Social status UI');
...
function rq_once(&$code,$old,$new,$label){ if(strpos($code,$new)!==false) return; $n=substr_count($code,$old); if($n!==1){fwrite(STDERR,"{$label}: trecho esperado count={$n}; abortando\n");exit(2);} $code=str_replace($old,$new,$code); }
...
$old="\$jobs[\$idx]['heygen_payload']=\$r['payload']; \$jobs[\$idx]['status']='heygen_agente_processando'; \$jobs[\$idx]['updated_at']=date('c'); rpia_write('videos_ia.json',\$jobs);";
$new="\$jobs[\$idx]['heygen_payload']=\$r['payload']; \$jobs[\$idx]['status']='heygen_agente_processando'; unset(\$jobs[\$idx]['send_lock_at']); \$jobs[\$idx]['updated_at']=date('c'); rpia_write('videos_ia.json',\$jobs);";
...
$code=file_get_contents($path);
if($code===false){fwrite(STDERR,"Reporter IA não encontrado\n");exit(1);}
function rdh_once(&$code,$old,$new,$label){ if(strpos($code,$new)!==false) return; $n=substr_count($code,$old); if($n!==1){fwrite(STDERR,"{$label}: trecho esperado count={$n}; abortando\n");exit(2);} $code=str_replace($old,$new,$code); }
...
def main() -> None:
    ...
    for path, content in files.items():
        ...
        if old == content:
            continue
        backup = path.with_name(f'{path.name}.backup-adaptive2-{STAMP}')
        shutil.copy2(path, backup)
        backups.append(backup)
        path.write_text(content, encoding='utf-8')
    print('TVSUMARE_BUILD_CHAIN_ADAPTIVE_V2_INSTALLED')
    for item in backups: print(f'BACKUP={item}')
