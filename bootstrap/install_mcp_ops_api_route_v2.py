...
def backup(path: Path) -> None:
    if path.exists():
        shutil.copy2(path, path.with_name(f'{path.name}.backup-ops-api-route-v2-{STAMP}'))
...
def main() -> None:
    ...
    # Import compatibility test: catches missing wrappers before Docker rebuild.
    run(['python3', '-c', 'import project_manager_tools; import main; print("IMPORT_COMPATIBILITY=OK")'], LIVE_ROOT)
    ...
    print('MCP_OPS_API_ROUTE_V2_INSTALLED=SIM')
    print(f'BACKUP_STAMP={STAMP}')
    print('STRATEGY=in_place_transport_patch')
